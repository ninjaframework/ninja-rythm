...
import ninja.rythm.util.RythmConstant;
...
public class TemplateEngineRythm implements TemplateEngine {
    ...
    private final RythmHelper rythmHelper;
    ...
    public void invoke(Context context, Result result) {
        ...
        String templateName = rythmHelper.getRythmTemplateForResult(
        ...
        try {
            ...
            if (templateName.equals(response)) {
                handleServerError(context, new Exception(
                        "Couldn't locate template " + templateName + " in views."));
            } else {
    ...
    private void handleServerError(Context context, Exception e) {
        ...
        String response = engine.getTemplate(
                RythmConstant.LOCATION_VIEW_HTML_INTERNAL_SERVER_ERROR)
                .render();
