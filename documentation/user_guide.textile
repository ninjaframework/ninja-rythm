...
Before Rythm v1.0 you needed to pass the full path to extended template file starting from @app/rythm@ folder: <code>@extends("main.html")</code>
...
|| Variable || Description || Example usage in template ||
| @lang@ | Language | <code><li>Implicit language is: @lang</li> </code>|
| @session@ | Session. From controller <code>context.getSessionCookie().put("username", "kevin");</code> | <code><b>@session.get("username")</b></code> |
| @contextPath@ | Get the context path on which the application is running, ie. return the context-path with a leading "/" or "" if running on root | <code><b>@contextPath</b></code> |
...
| @custom_flash@ | <code>context.getFlashCookie().put("custom_flash", "some custome msg");</code> |<code> @flash.get("custom_flash")</code>|
